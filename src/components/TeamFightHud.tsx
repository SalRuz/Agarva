...
export function TeamFightHud({ mode, phase, countdown, fightSecondsLeft, blue, red }: TeamFightHudProps) {
  ...
  return <div className="absolute top-4 right-4 z-30 select-none pointer-events-none">
    <div className="bg-black/75 backdrop-blur-sm rounded-lg px-4 py-3 min-w-[230px] border border-violet-500/30">
      ...
      <div className="flex justify-between gap-4 text-sm">
        <div className="text-blue-300">Синие {blue.alive}/{blue.total}<div className="text-white/70 max-w-[95px] truncate">{blue.members.join(', ') || '—'}</div></div>
        <div className="text-red-300 text-right">Красные {red.alive}/{red.total}<div className="text-white/70 max-w-[95px] truncate">{red.members.join(', ') || '—'}</div></div>
      </div>
    </div>
  </div>;
}
