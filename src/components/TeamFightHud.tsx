  ...
  return <div className="absolute top-4 right-4 z-30 select-none pointer-events-none">
    <div className="bg-black/75 backdrop-blur-sm rounded-lg px-4 py-3 min-w-[230px] border border-violet-500/30">
      ...
      <div className="grid grid-cols-2 gap-4 text-sm">
        <div className="min-w-0 text-blue-300">
          <div>Синие {blue.alive}/{blue.total}</div>
          <div className="mt-1 flex flex-col gap-0.5 whitespace-normal break-words text-white/70">
            {blue.members.length ? blue.members.map((name) => <div key={name}>{name} 🔥{blue.streaks[name] ?? 0}</div>) : '—'}
          </div>
        </div>
        <div className="min-w-0 text-right text-red-300">
          <div>Красные {red.alive}/{red.total}</div>
          <div className="mt-1 flex flex-col gap-0.5 whitespace-normal break-words text-white/70">
            {red.members.length ? red.members.map((name) => <div key={name}>{name} 🔥{red.streaks[name] ?? 0}</div>) : '—'}
          </div>
        </div>
      </div>
